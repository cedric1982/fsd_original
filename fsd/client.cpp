   ...
   char *rev, char *real, int st)
{
   ...
   // --------------------------------------------------------------------
   // Swift / CQ state cache + debounce state (needed by cluser.cpp CQ logic)
   // --------------------------------------------------------------------
   has_cq = 0;
   cq_ts  = 0;
   if (sizeof(last_cq) > 0) last_cq[0] = '\0';
...
void client::updatepilot(char **array)
{
   ...
   if (lat>90.0||lat<-90.0||lon>180.0||lon<-180.0)
      dolog(L_DEBUG, "POSERR: s=(%s,%s) got(%f,%f)", array[4], array[5], lat, lon);
